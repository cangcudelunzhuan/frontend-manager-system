<template lang="pug">
  div
    .filter-btn-wrap
      a-button(@click="add" icon="plus-circle" type="primary") 新增产品
    tableList(:columnsData="columns" :tabledata="data" :loading="loading" :page="pagination"  @tablechange="fetch" )
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    async fetch (params) {
      this.loading = true
      try {
        let res = await api.productList(params || {})
